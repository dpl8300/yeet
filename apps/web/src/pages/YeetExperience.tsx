...
function LeaderboardPreview({ snapshot, loading, failed, onOpen }: { snapshot?: LeaderboardSnapshot; loading: boolean; failed: boolean; onOpen: () => void }) {
  const entries = snapshot?.leaders.slice(0, 3) ?? [];
  return (
    <div className="leaderboard-card">
      ...
      {!entries.length && <button className="empty-preview" onClick={onOpen}>{failed || !isSupabaseConfigured ? "LEADERBOARD UNAVAILABLE" : loading ? "LOADING SCORES…" : "BE FIRST ON THE BOARD"}</button>}
      {entries.map((entry) => <button className="leader-row" onClick={onOpen} key={entry.user_id}><strong>{String(entry.rank).padStart(2, "0")}</strong><span>@{entry.handle}</span><b>{formatSeconds(entry.airtime_ms)}</b></button>)}
    </div>
  );
}
