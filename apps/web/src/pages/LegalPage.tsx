...
import { LEGAL_EFFECTIVE_DATE, LEGAL_EFFECTIVE_ISO_DATE } from "../lib/legal";
  ...
  return (
    <main className="legal-shell">
      <article className="legal-card">
        ...
        <p className="eyebrow">YEET · EFFECTIVE <time dateTime={LEGAL_EFFECTIVE_ISO_DATE}>{LEGAL_EFFECTIVE_DATE.toUpperCase()}</time></p>
...
function Privacy() {
  return <>
    ...
    <h2>AUTOMATIC, ANALYTICS, AND SERVICE DATA</h2><p>YEET and its service providers may process IP address, browser and device details, timestamps, request metadata, security events, and diagnostic or delivery logs when your device connects to the service. Vercel Web Analytics begins when the app loads, including before you acknowledge the Terms, and records anonymous page views and aggregated information such as the page path, referrer, general location, operating system, browser, and device type. YEET removes query strings and fragments from analytics URLs before transmission. Vercel identifies a daily visitor using a request-derived hash rather than cookies and discards that visitor identifier after 24 hours. Loading Google Fonts also sends ordinary request data to Google.</p>

    <h2>ANONYMOUS INTERACTION EVENTS</h2><p>YEET sends categorical events to Vercel when the legal gate is displayed or accepted; when home, settings, account, or leaderboard screens are opened; when sign-in, profile, sign-out, or deletion steps succeed or fail; when a YEET is started, prepared, detected, completed, retried, or rejected; when a guest rank estimate or signed-in score save succeeds or fails; and when POV recording, playback, export, sharing, or app installation is used. Event properties may include guest or signed-in account state, whether a signed-in player has a handle or saved score, browser-versus-installed display mode, broad airtime and rank ranges, feature state, interaction source, and fixed failure categories. YEET does not send Vercel your user or attempt identifier, email address, handle, exact airtime, raw motion trace, POV recording, authentication code, or provider error message.</p>

    <h2>HOW INFORMATION IS USED</h2><p>YEET uses information to run and secure gameplay, authenticate accounts, validate and rank attempts, display the leaderboard, understand aggregate traffic and product funnels, measure feature adoption and failure rates, remember preferences and consent, deliver sign-in messages, provide support, prevent abuse, diagnose failures, enforce the Terms, and comply with law.</p>

    <h2>SERVICE PROVIDERS AND DISCLOSURE</h2><p>Information is processed as needed by Vercel for web hosting and privacy-focused page-view and custom-event analytics, Supabase for authentication and leaderboard services, Resend for authentication and outbound support email, Namecheap for support-email forwarding, the destination support-mail provider, and Google for OAuth and hosted fonts. YEET may also disclose information when reasonably necessary to comply with law, protect users or the public, investigate abuse, or complete a business transfer subject to this Notice.</p>

    <h2>NO SALE, ADS, OR CROSS-SITE TRACKING</h2><p>YEET does not use advertising analytics, sell personal information, or share it for cross-context behavioral advertising. Vercel Web Analytics is cookie-free and reports aggregated page and interaction data without identifying you across different days or websites. Because YEET does not sell, share for behavioral advertising, or track your activity across unrelated websites, browser Do Not Track and Global Privacy Control signals do not change the service’s current behavior.</p>

    <h2>RETENTION</h2><p>Account and derived attempt data are retained while your account exists. Raw ranked-submission traces are used transiently and are not stored as database records. Temporary POV data lasts only for the current attempt unless you save or share it. Vercel retains aggregated page-view and custom-event analytics according to the project’s plan and settings, while its daily visitor identifier expires after 24 hours. Support communications, authentication records, and operational logs are retained only as reasonably needed for support, security, legal obligations, and provider operations.</p>
...
function Terms() {
  return <>
    ...
    <h2>PRIVACY, ANALYTICS, AND PROVIDERS</h2><p>YEET processes information as described in the Privacy Notice, including through service providers that host, secure, authenticate, support, and measure use of the service. YEET uses Vercel Web Analytics for anonymous page views and categorical product-interaction events used to understand funnels, feature adoption, and reliability without third-party analytics cookies, persistent user identifiers, or cross-site behavioral tracking. Third-party services you choose to use, including OAuth, email, and sharing destinations, may apply their own terms and privacy practices.</p>
